...
public class HomePageFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedIstanceState) {
		...
		Typeface robotoli = Typeface.createFromAsset(getActivity().getAssets(), "fonts/Roboto-LightItalic.ttf");
		Typeface robotob = Typeface.createFromAsset(getActivity().getAssets(), "fonts/Roboto-Bold.ttf");
...
//		tv_header = inflater.inflate(R.layout.header_tv, null);
		...
		ParallaxTransformer pt = new ParallaxTransformer(R.id.imageView2,R.id.imageView1);
		...
		protected View getBucketElement(final int position, String currentElement) {
			...
			bucketElement.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					((MainActivity)getActivity()).mActionBarHelper = new FadingActionBarHelper(getActivity().getActionBar(),getActivity().getResources().getDrawable(R.drawable.actionbar_bg));
					((MainActivity)getActivity()).getSupportFragmentManager().beginTransaction().add(R.id.content_frame, new TestFragment()).hide(frag).addToBackStack("testing").commit();
